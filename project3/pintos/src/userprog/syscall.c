...
syscall_handler (struct intr_frame *f UNUSED) 
{
  ...
  switch(* (int *) f->esp) {
    ...
    case SYS_WRITE: {     
      get_arguments(f, &arg[0],3);
      check_buffer((const void *)arg[1], (unsigned) arg[2], f);
      f->eax = write(arg[0], arg[1], (unsigned) arg[2]);
      break;
    }
    case SYS_SEEK: {    
      get_arguments(f, &arg[0],2); 
...
get_arguments (struct intr_frame *f, int *arg, int n)
{
  ...
  for(i = 0; i < n; i++) {
    ...
    if(!is_valid_ptr((const void*) ptr, f))
      exit(-1);
    arg[i] = *ptr;
  }
}
...
check_writable(const void *buffer, unsigned size, struct intr_frame *f) {
  ...
  for(i = 0; i < size; i++) {
    struct spt_entry *spte = get_spte(buffer+i);
    if(!spte->writable)
      exit(-1);
  }
}
  ...
  for( ; e != list_end(&t->children); e = list_next(e)) {
    ...
    if (cp->loaded == 0) {
      /* Wait for the sema_exec. If the executable file has been
         loaded, sema_exec will be sema_up, then the process can 
         be continued. */
...
open (const char *file)
{
  ...
  /* Set the fd. Every time it is opened_files length puls 2. 
     Make sure every file has different fd. */
  int size = list_size(&cur->opened_files);
  if(size == 0)
    holder->fd = 2;
  else {
...
mmap(int fd, void *addr)
{
  ...
  while(total_bytes > 0){
    ...
    // call create_mmap_page_table, add page to page table
    if(!create_mmap_page_table(file_copy, ofs, addr,
		               read_bytes, zero_bytes)){
      //if failed, unmap file
      munmap(t->mapid);
      return -1;
    }
...
munmap(int mapping)
{
  ...
  while(e != list_end(&t->mmap_list)){
    ...
    // mapind is mateched
    // or mapping is 0, which is called in process_exit(), unmap all the pages
    if(mme->mapid == mapping || mapping == 0) {
      spte->pinned = true;
      if(spte->loaded) {
	...
	// free frame and clear page
      ...
      if(mme->mapid != current_id) {
	if(f) {
          lock_acquire(&file_lock);
          file_close(f);
          lock_release(&file_lock);
         }
	current_id = mme->mapid;
        f = spte->file;
      }
